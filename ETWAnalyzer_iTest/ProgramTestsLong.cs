...
namespace ETWAnalyzer_iTest
{
    ...
    public class ProgramTestsLong
    {
     ...
     //           extractArgs = extractArgs.Concat(new string[] { "-symserver", "MS" }).ToArray();
        ...
        /// <summary>
        /// Compare data generated by TraceLoggingTester. If you want to update the data run TraceLoggingTester to regenerate the data
        /// and import TraceLoggingtester.7z into ETWAnalyzer_uTest/TestData
        /// </summary>
        [Fact]
        public void Can_Extract_TraceLoggingData()
        {
            Program.DebugOutput = false;
            using var tmp = TempDir.Create();
            string pathName = Path.Combine(tmp.Name, Path.GetFileName(TestData.TraceLoggingTesterEtlFile));
            File.Copy(TestData.TraceLoggingTesterEtlFile, pathName);

            Program.MainCore(new string[] { "-extract", "TraceLog", "-noCompress", "-indent", "-filedir", tmp.Name, "-outdir", tmp.Name });

            string outFile1 = GetExtractFile(tmp, pathName);
            var fileInfo1 = new FileInfo(outFile1);

            Assert.True(fileInfo1.Exists, $"Output file {outFile1} was not created");
            Assert.True(fileInfo1.Length > 0, $"File {outFile1} has no content");

            TestRunData runData = new TestRunData(outFile1);
            var extract = runData.AllFiles[0].Extract;

            Assert.Single(extract.TraceLogging.EventsByProvider);
            Assert.Equal("CSharpTraceLoggingEventSource", extract.TraceLogging.EventsByProvider["CSharpTraceLoggingEventSource"].ProviderName);

            ITraceLoggingData tracelog = extract.TraceLogging;

            Assert.Single(tracelog.EventsByProvider);

            foreach (var prov in tracelog.EventsByProvider)
            {
                Assert.Equal("CSharpTraceLoggingEventSource", prov.Key);

                Assert.Equal(70, prov.Value.Events.Count);

                var stringEv = prov.Value.Events[0];
                var stringIntEv = prov.Value.Events[1];
                var complexEv = prov.Value.Events[2];
                var listEv = prov.Value.Events[3];
                var dateTimeEv = prov.Value.Events[4];
