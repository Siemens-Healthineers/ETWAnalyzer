...
namespace ETWAnalyzer.Extract
{
    ...
    public class ProcessWorkingSet : IProcessWorkingSet
    {
        ...
        /// <param name="commitMB"></param>
        /// <param name="workingsetMB"></param>
        /// <param name="workingsetPrivateMB"></param>
        /// <param name="sharedCommitSizeMB"></param>
        /// <exception cref="ArgumentNullException">When process is null</exception>
        internal ProcessWorkingSet(ProcessKey process, decimal commitMB, decimal workingsetMB, decimal workingsetPrivateMB, decimal sharedCommitSizeMB)
        {
            if (process == null)
            {
            ...
            CommitInMiB = (ulong)Math.Round(commitMB, 0, MidpointRounding.AwayFromZero);
            WorkingSetInMiB = (ulong)Math.Round(workingsetMB, 0, MidpointRounding.AwayFromZero);
            WorkingsetPrivateInMiB = (ulong)Math.Round(workingsetPrivateMB, 0, MidpointRounding.AwayFromZero);
            SharedCommitSizeInMiB = (ulong)Math.Round(sharedCommitSizeMB, 0, MidpointRounding.AwayFromZero);
        }
