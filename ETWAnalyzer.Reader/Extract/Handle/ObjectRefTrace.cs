...
namespace ETWAnalyzer.Extract.Handle
{
    ...
    public class ObjectRefTrace : IObjectRefTrace
    {
            ...
            get
            {
                ...
                if (first == null)
                {
                    if (IsFileMap && FileMapEvents?.Count > 0)
                    {
                        ...
                        first = new RefCountChangeEvent(0, 1, firstMap.ProcessIdx, firstMap.ThreadId, firstMap.StackIdx)
                        {
                    ...
                    else if (HandleCreateEvents?.Count > 0)
                    {
                        ...
                        first = new RefCountChangeEvent(0, 1, firstHandleCreate.ProcessIdx, firstHandleCreate.ThreadId, firstHandleCreate.StackIdx)
                        {
            ...
            get
            {
                ...
                if (last == null)
                {
                    if (IsFileMap && FileUnmapEvents.Count > 0)
                    {
                        ...
                        last = new RefCountChangeEvent(0, -1, lastUnmap.ProcessIdx, lastUnmap.ThreadId, lastUnmap.StackIdx)
                        {
                    ...
                    } else if (HandleCloseEvents.Count > 0 && (HandleCreateEvents.Count - (HandleCloseEvents.Count + HandleDuplicateEvents.Count) == 0))
                    {
                        ...
                        last = new RefCountChangeEvent(0, -1, lastClose.ProcessIdx, lastClose.ThreadId, lastClose.StackIdx)
                        {
        ...
        internal void AddRefChange(long timeNs, int refCountChange, ETWProcessIndex process, uint threadId, StackIdx idx)
        {
            RefChanges.Add(new RefCountChangeEvent(timeNs, refCountChange, process, threadId, idx));
        }

        internal void AddHandlCreate(long timeNs, ulong handleValue, ETWProcessIndex processIdx, uint threadId, StackIdx stackIdx)
        {
            var created = new HandleCreateEvent(timeNs, handleValue, processIdx, threadId, stackIdx);
            HandleCreateEvents.Add(created);
        }

        internal void AddHandleDuplicate(long timeNs, uint sourceHandle, uint targetHandle, ETWProcessIndex processIdx, ETWProcessIndex sourceProcessIndex, uint threadId, StackIdx stackIndex)
        {
            var duplicate = new HandleDuplicateEvent(timeNs, targetHandle, sourceHandle, processIdx, sourceProcessIndex, threadId, stackIndex);
            HandleDuplicateEvents.Add(duplicate);
        }

        internal bool AddHandleClose(long timeNs, ulong handleValue, string handleName, ETWProcessIndex processIdx, uint threadId, StackIdx stackIdx)
        {
            ...
            var closed = new HandleCloseEvent(timeNs, handleValue, processIdx, threadId, stackIdx);
        ...
        internal void AddFileMap(long timeNs, long viewBase, long viewSize, long fileObject, long byteOffset, ETWProcessIndex processIdx, uint threadId, StackIdx stackIdx)
        {
        ...
        internal void AddFileUnMap(long timeNs, long viewBase, long viewSize, long fileObject, long byteOffset, ETWProcessIndex processIdx, uint threadId, StackIdx stackIdx)
        {
