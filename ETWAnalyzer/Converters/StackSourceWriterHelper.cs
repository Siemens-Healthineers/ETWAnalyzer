...
{
    /// <summary>
    /// Adapted from https://github.com/microsoft/perfview/blob/main/src/TraceEvent/Stacks/StackSourceWriterHelper.cs
    /// Added culture invariant parsing and stringification
    /// </summary>
    internal static class StackSourceWriterHelper
    {
        ...
        internal static IReadOnlyDictionary<ThreadInfo, List<Sample>> GetSortedSamplesPerThread(StackSource stackSource)
        {
            ...
            stackSource.ForEach(sample =>
            {
                ...
                while (stackIndex != StackSourceCallStackIndex.Invalid)
                {
                    ...
                    if (!frameName.StartsWith("Thread (", StringComparison.Ordinal))
                    {
        ...
        internal static bool Validate(IReadOnlyList<ProfileEvent> orderedProfileEvents)
        {
            ...
            foreach (var current in orderedProfileEvents)
            {
                ...
                if (double.Parse(previous.RelativeTime.ToString("R", CultureInfo.InvariantCulture), CultureInfo.InvariantCulture) > double.Parse(current.RelativeTime.ToString("R", CultureInfo.InvariantCulture), CultureInfo.InvariantCulture))
                {
                    return false;
                }
        ...
        internal readonly struct ThreadInfo : IEquatable<ThreadInfo>
        {
            ...
            internal ThreadInfo(string threadFrameName, string processFrameName)
            {
                ...
                Id = threadIdMatch.Success ? int.Parse(threadIdMatch.Groups[1].Value, CultureInfo.InvariantCulture) : 0;
                ProcessId = processIdMatch.Success ? int.Parse(processIdMatch.Groups[1].Value, CultureInfo.InvariantCulture) : 0;
            }
